...
The dataset for this assignment can be found as a csv file in the `data` folder of your repository. You can read it in by running the following code in the Console:
...
You should be able to complete this section in 5 minutes or less.
...
One country stands out as an unusual observation at the top of the distribution (meaning, the right side of the histogram). One way of identifying this country is to filter the data for countries where plastic waste per capita is greater than 3.5 kg/person.
...
```{marginfigure}
**Note:** Be sure to always leave a blank line of whitespace in between chunks or statements in an R Markdown document. Otherwise, it will not render properly.
```
...
```{marginfigure}
Another hint: To make the background white instead of gray, add in the layer `theme_minimal()`
```
