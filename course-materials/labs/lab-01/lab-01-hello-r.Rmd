...
At the start of each session, you will be given a 5-minute warm-up exercise to discuss with your group. This time, use this as an opportunity to meet your course mates. What's everyone's favorite emoji and why? And the least favorite?
...
- **Repository**: A Git repository repository contains all of your project's files and stores each file's revision history. It's common to refer to a repository as a repo. 
  ...
  - All repos associated with this course are housed in a GitHub organization: [`rcnj-ids-fa20`](https://github.com/rcnj-ids-fa20). The organization is set up such that students can only see repos they have access to, but the course staff can see all of them.
...
- Go to RStudio Cloud and into the **course workspace, Data-101-fall-2020** (not "Your Workspace"). Click on the down arrow next to the **New Project** button and select **New Project from Git Repo**.
...
Below are the general components of an RMarkdown file - this picture is just to get you oriented. 
...
Load these packages by running the following in the Console (type or paste each line in the Console, pressing Enter after each).
...
Run the following (but update it for your name and email!) in the Console to configure git. Be sure to use the same email address you used for your GitHub registration.
