...
```{r fig.margin=TRUE, eval=TRUE, echo=FALSE, fig.cap="Photo by Marcelo Leal on Unsplash"}
knitr::include_graphics("img/med-pic-unsplash.jpg")
```
...
```{marginfigure}
**Hint:** Use the `as_factor()` function
```

1. Modify `icu` so that the binary variables `survive`, `sex`, `infection`, and `emergency` are treated as factors. Then `glimpse` the data frame to confirm your data types are correct.
...
```{marginfigure}
**Hint:** you will need to use the inverse-logit function.
```

1. Use your answer from the previous question to find the patient's predicted probability of survival, $p$. Round the probability to 2 decimal places.
...
*This lab was derived from an activity written by Dr. Robert Eisinger, Duke University.*
