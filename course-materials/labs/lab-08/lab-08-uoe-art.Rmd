...
```{marginfigure}
See the sidebar [here](https://collections.ed.ac.uk/art) and note that there are 2960 pieces in the art collection we're collecting data on.
```
...
Go to the course GitHub organization and locate your lab repo, which should be named `lab-08-uoe-art-YOUR_TEAMNAME`. Grab the URL of the repo, and clone it in our RStudio Cloud **course workspace**.
...
```{marginfigure}
**Tip:** To run the code in an R script, you can highlight or put your cursor next to the lines of code you want to run and press Command+Enter.
```
...
We will start by scraping data on the first 10 pieces in the collection, found 
...
```{marginfigure}
**Reminder:** The collection has 2960 pieces in total.
```
...
<div class="box">
Work in the file `lab-06-uoe-art.Rmd` for the rest of the lab.
</div>
