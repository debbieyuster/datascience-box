...
- `inner_join()`: all rows from x where there are matching values in y, returns 
...
.your-turn[
- RStudio Cloud > `AE 05 - Fisheries + Data joins` > open `fisheries.Rmd`.
- Knit the document and work on Exercises 1 - 4.
]
...
```{r echo=FALSE, fig.asp=0.8, out.width="75%"}
ggplot() +
  geom_polygon(world_map, 
               mapping = aes(x = long, y = lat, group = group), 
               fill = "lightgray") +
