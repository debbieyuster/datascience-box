---
title: "Modeling non-linear relationships <br> `r emo::ji('wavy_dash')`"
author: ""
output:
...
.pull-right[
```{r fig.width=5, fig.height=4, message=FALSE, echo=FALSE}
pp_wt_lt_100 <- pp_wt_lt_100 %>% mutate(log_price = log(price))
m_lprice_wt <- lm(log_price ~ Width_in, data = pp_wt_lt_100)
m_lprice_wt_aug <- augment(m_lprice_wt)
ggplot(data = m_lprice_wt_aug, 
       mapping = aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.5) +
  labs(
...
For each additional inch the painting is wider, the price of the
painting is expected to be higher, on average, by a factor of 1.02. In other words, the price is expected to increase, on average, by 2%.
...
$$ \widehat{log(price)} = 4.67 + 0.02 Width $$
Intercept is the value of the response variable when the predictor variable is 0:
$$ \widehat{log(price)} = 4.67 + 0.02(0) $$
--

$$ \widehat{log(price)} = 4.67$$
--

$$ e^{\widehat{log(price)}} = e^{4.67}$$
--

$$ \widehat{price} = e^{4.67} = 107$$
...
the slope, $b_1$, changes: *"For each unit increase in x, y is expected on average to be higher/lower by a factor of $e^{b_1}$."*
