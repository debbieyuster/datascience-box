...
- Response variable: `log_price` = log(price)
...
Less than 1000 square inches (~ 32in. x 32in.). There are very few paintings that have surface area above 5000 square inches.
...
**Goal**: Predict a book's weight as a function of its volume and cover type.
...
- All else held constant, for each additional square inch in painting's surface area, the price of the painting is predicted, on average, to be higher by a factor of 1.000265 (= $e^{0.000265}$).
...
+ Same slope and different intercept for each color
+ Different slope and different intercept for each color
]
...
m_int <- lm(log_price ~ Surface + artistliving + Surface * artistliving, 
            data = pp_Surf_lt_5000)
tidy(m_int)
```
]
